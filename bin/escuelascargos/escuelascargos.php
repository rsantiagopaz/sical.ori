...
switch ($_REQUEST['rutina'])
{
	case 'insert': 
	{
		...
		{	
			...
			if($row['cc1']!=0) {
				...
				$sql="SELECT denominacion FROM cargos WHERE id_cargo = '$id_cargo'";
				$result=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
				$row=mysqli_fetch_array($result);
				$cargo = $row['denominacion'];
				
				$sql="INSERT escuelas_cargos SET id_escuela = '".$xml_Escuela["id_escuela"]."', cod_escuela = '".$xml_Escuela["codigo"]."', " .
					"id_cargo='$id_cargo', cod_cargo='".$xml_Escuela['cod_cargo']."'";			
				toXML($xml, $sql, "add");
   ...
   case 'traer_escuelas':
 	{ 	 		
		$xml=new SimpleXMLElement('<rows/>');
	  	$sql="SELECT id_escuela_cargo, id_cargo, cod_cargo, ca.denominacion 'denomcar', id_escuela, cod_escuela 'codigo', nombre, n.id_nivel 'id_nivel', n.nivel 'nivel', 'A' as origen ";
		$sql.="FROM escuelas_cargos ";
