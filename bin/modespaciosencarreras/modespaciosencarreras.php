...
switch ($_REQUEST['rutina'])
{
	case 'insert': 
	{
		...
		if($n["cc"]==0)
		{
			...
			$sql="SELECT denominacion FROM espacios WHERE id_espacio = '".$xml_Espacio['id_espacio']."'";
			$result=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
			$row=mysqli_fetch_array($result);
			$espacio = $row['denominacion'];
			
			$sql="INSERT carreras_espacios SET id_carrera = '$id_carrera', cod_carrera = '$cod_carrera', " .
					"id_espacio='".$xml_Espacio['id_espacio']."', cod_espacio='".$xml_Espacio['codigo']."'";			
			toXML($xml, $sql, "add");						
	...
	case 'delete': 
	{
		...
		$xml=new SimpleXMLElement('<rows/>');
		$sql="SELECT nombre FROM carreras WHERE id_carrera = '".$xml_Espacio['id_carrera']."'";
		$result=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
		$row=mysqli_fetch_array($result);
		$carrera = $row['nombre'];
		...
		$sql="DELETE FROM carreras_espacios ";
		$sql.="WHERE id_carrera_espacio='".$xml_Espacio["id_carrera_espacio"]."' ";
		//$sql.="AND id_carrera='$id_carrera'";
		toXML($xml, $sql, "del");
		
		if( !(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_errno($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false))>0) ) {
			$_id_registro=$xml_Espacio["id_carrera_espacio"];
			$_descrip = "BAJA DE LA CARRERA-ESPACIO CON id='$_id_registro', CARRERA '$carrera', ESPACIO '$espacio'";
			_auditoria($sql, 
                    $link_salud1,
					$link_salud1,
					'carreras_espacios',
   ...
   case 'traer_espacios':
 	{ 	 		
		$xml=new SimpleXMLElement('<rows/>');	  	
	  	$sql="SELECT id_carrera_espacio, id_espacio, codigo, denominacion, 'A' as origen ";
		$sql.="FROM carreras_espacios ";
