...
switch ($_REQUEST['rutina'])
{
	case 'listado_titulos':
	{
		...
		$objPHPExcel->getProperties()->setCreator("SiCal")
						 ...
						 ->setTitle("Títulos por Espacio")
						 ->setSubject("Títulos por Espacio");
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 'Títulos por Espacio');
		...
		$objPHPExcel->getActiveSheet()->getStyle('A3:E3')->applyFromArray($styleFill);
	    ...
	    $objPHPExcel->getActiveSheet()->setCellValue('A3','ESPACIO');
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':E'.$i)->applyFromArray($styleFill);
		...
		$objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue('E' . $i,'Carrera');
		
		while ($row = mysqli_fetch_array($result)) {		
			$i++;
			
			$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':E'.$i)->applyFromArray($styleNoFill);																
